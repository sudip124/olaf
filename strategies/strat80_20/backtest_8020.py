...
class Backtest8020(BacktestStrategy):
    """Backtest strategy for strat80_20."""
        ...
        **kwargs
        ...
        for symbol in symbols:
            ...
            try:
                ...
                setup_df = scanner.get_setup_days(symbol, from_date, to_date, interval=scan_interval, return_details=True)
        ...
        journal_df = pd.DataFrame()  # Initialize journal dataframe
        try:
        ...
        # Build journal dataframe using the strategy-specific JournalBuilder
        try:
            if backtest_run_id is not None:
                builder = JournalBuilder8020(strategy_name='strat80_20')
                journal_df = builder.build(backtest_run_id)
                print(f"[Backtest] Built journal DataFrame with {len(journal_df)} row(s)")
            else:
                print("[Backtest] Warning: backtest_run_id is None, cannot build journal DataFrame")
        except Exception as e:
            print(f"[Backtest] Warning: failed to build journal DataFrame: {e}")
        ...
        return journal_df, backtest_run_id


# Backward compatibility: provide run_backtest function
def run_backtest(symbols, from_date, to_date, scan_interval='D', backtest_interval='15m', optimize=False, max_attempts=None):
    """
    Backward compatibility wrapper for run_backtest function.
    Creates a Backtest8020 instance and calls its run_backtest method.
    """
    strategy = Backtest8020()
